...
class AnimalMatch extends Game {
  ...
  draw() {
    this.clear(); this.ctx.fillStyle = '#fef9e7'; this.ctx.fillRect(0, 0, this.width, this.height);
    for (let r = 0; r < this.rows; r++) {
      for (let c = 0; c < this.cols; c++) {
    ...
    this.ctx.fillStyle = '#d35400'; this.ctx.font = '14px sans-serif'; this.ctx.textAlign = 'center';
  ...
  cheat(action) { if (action === 'auto') { this.grid = this.grid.map(row => row.map(() => Math.floor(Math.random() * this.icons.length))); this.draw(); } }
  getState() { return { score: this.score }; }
  restart() { this.init(); }
}

module.exports = AnimalMatch;
