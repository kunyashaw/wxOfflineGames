...
class CandyMatch extends Game {
  ...
  init() {
    ...
    for (let r = 0; r < this.rows; r++) {
      for (let c = 0; c < this.cols; c++) { this.grid[r][c] = Math.floor(Math.random() * this.candies.length); }
    }
    this.score = 0;
    this.updateScore(0);
    this.draw();
  ...
  removeMatches(matches) {
    matches.forEach(([r, c]) => { this.grid[r][c] = -1; this.score += 10; });
  ...
  drop() {
    for (let c = 0; c < this.cols; c++) {
      ...
      for (let r = this.rows - 1; r >= 0; r--) {
        ...
        else if (empty.length > 0) { this.grid[empty.shift()][c] = this.grid[r][c]; this.grid[r][c] = -1; empty.push(r); }
      }
      empty.forEach(r => { this.grid[r][c] = Math.floor(Math.random() * this.candies.length); });
    }
  ...
  draw() {
    this.clear();
    this.drawRect(0, 0, this.width, this.height, '#ff9ff3');
    for (let r = 0; r < this.rows; r++) {
      for (let c = 0; c < this.cols; c++) {
        const x = this.offsetX + c * this.cellSize;
        const y = this.offsetY + r * this.cellSize;
        this.drawText(this.candies[this.grid[r][c]], x + this.cellSize / 2, y + this.cellSize / 2, 24);
      }
    }
  ...
  onTouchEnd(e) {
    ...
    if (!this.selected) { if (r >= 0 && r < this.rows && c >= 0 && c < this.cols) this.selected = { r, c }; }
      ...
      if (Math.abs(this.selected.r - r) + Math.abs(this.selected.c - c) === 1) this.swap(this.selected.r, this.selected.c, r, c);
      this.selected = null;
    }
  }

  cheat(action) { if (action === 'auto') { this.grid = this.grid.map(row => row.map(() => Math.floor(Math.random() * this.candies.length))); this.draw(); } }
  getState() { return { score: this.score }; }
  restart() { this.init(); }
}

module.exports = CandyMatch;
