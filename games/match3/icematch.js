...
class IceMatch extends Game {
  ...
  init() {
    ...
    for (let r = 0; r < this.rows; r++) {
      for (let c = 0; c < this.cols; c++) { this.grid[r][c] = Math.floor(Math.random() * this.icons.length); }
    }
    this.score = 0;
    this.selected = null;
    this.updateScore(0);
  ...
  draw() {
    this.clear();
    this.drawRect(0, 0, this.width, this.height, '#d5f5e3');
    for (let r = 0; r < this.rows; r++) {
      for (let c = 0; c < this.cols; c++) {
        const x = this.offsetX + c * this.cellSize;
        const y = this.offsetY + r * this.cellSize;
        this.drawText(this.icons[this.grid[r][c]], x + this.cellSize / 2, y + this.cellSize / 2, 22);
      }
    }
    this.drawText(`得分: ${this.score}`, this.width / 2, 20, 14, '#27ae60');
  }
